...
tchunk1 = year+'-03-25'
tchunk2 = year+'-04-10'
T_meas = da_temp.sel(time=slice(tchunk1, tchunk2)).mean('time')
...
axx[0].plot([np.datetime64(tchunk1), np.datetime64(tchunk1)], [z.min(), z.max()], 'w--')
axx[0].plot([np.datetime64(tchunk2), np.datetime64(tchunk2)], [z.min(), z.max()], 'w--')
...
axx[1].plot([np.datetime64(tchunk1), np.datetime64(tchunk1)], [z.min(), z.max()], 'w--')
axx[1].plot([np.datetime64(tchunk2), np.datetime64(tchunk2)], [z.min(), z.max()], 'w--')
axx[1].set_title('Corrected via smoothing spline')
axx[1].set_ylabel(r'$z$ (m)')
plt.colorbar(im1,ax=axx[0], label=r'$T$ ($^\circ$C)')
plt.colorbar(im1,ax=axx[1], label=r'$T$ ($^\circ$C)')
